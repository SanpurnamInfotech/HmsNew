...
const MentalIllnessMaster = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      showModal(`Mental Illness ${isEdit ? "updated" : "created"} successfully!`);
      resetForm();
      refresh();
    } else {
  ...
  const handleDelete = async () => {
    ...
    const result = await deleteItem(`${PATH}/delete/${selectedRow.mental_illness_code}/`);
  ...
  return (
    <div className="app-container">
      ...
      <div className="section-header">
        <h4 className="page-title">Mental Illness Master</h4>
        {!showForm && (
      ...
      {showForm && (
        <div className="form-container animate-in zoom-in-95 duration-200">
          <h6 className="form-section-title uppercase tracking-tighter">
            {isEdit ? "Update Illness Details" : "Add New Mental Illness"}
          </h6>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6" onSubmit={handleSubmit}>
            
            {/* CODE */}
            <div className="space-y-1.5">
              <label className="form-label">Illness Code</label>
              <input
                ...
                onChange={(e) => setFormData({ ...formData, mental_illness_code: e.target.value.toUpperCase() })}
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, mental_illness_name: e.target.value })}
              />
            </div>
            ...
            <div className="md:col-span-2 flex justify-end gap-3 border-t pt-8 mt-4" style={{ borderColor: "var(--border-color)" }}>
              <button type="submit" className="btn-primary px-12 py-3">
                {isEdit ? "Update Illness" : "Save Illness"}
              </button>
            ...
            <table className="w-full text-left">
              <thead>
                <tr>
                  ...
                  <th className="text-admin-th">Illness Name</th>
                ...
                {paginatedData.length > 0 ? (
                  [...paginatedData]
                    ...
                    <tr 
                      ...
                      onClick={() => setSelectedRow(selectedRow?.mental_illness_code === row.mental_illness_code ? null : row)}
                      className={`group cursor-pointer transition-colors ${selectedRow?.mental_illness_code === row.mental_illness_code ? "bg-emerald-500/10" : "hover:bg-emerald-500/5"}`}
                    >
                      <td className="px-6 py-4">
                        <div className={`selection-indicator ${selectedRow?.mental_illness_code === row.mental_illness_code ? "selection-indicator-active" : "group-hover:border-emerald-500/50"}`}>
                          {selectedRow?.mental_illness_code === row.mental_illness_code && <div className="selection-dot" />}
                        </div>
                      </td>
                      <td className="text-admin-td">{row.mental_illness_code}</td>
                      <td className="text-admin-td">{row.mental_illness_name}</td>
                  ...
                  <tr>
                    <td colSpan="5" className="px-6 py-24 text-center">
                      <FaBrain size={64} className="mb-6 mx-auto opacity-10 text-emerald-500 animate-pulse" />
                      <p className="text-xl font-black opacity-30 uppercase tracking-widest">No illness records found</p>
                    </td>
                  </tr>
                )}
              </tbody>
