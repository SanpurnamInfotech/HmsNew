...
const ComplaintMaster = () => {
  /* ================= DATA FETCHING ================= */
  const COMPLAINT_PATH = "complaints";
  const { data, loading, refresh, createItem, updateItem, deleteItem } = useCrud(`${COMPLAINT_PATH}/`);
  ...
  const resetForm = () => {
    ...
    setIsEdit(false);
    setSelectedRow(null);
    setFormData({ complaint_code: "", complaint_name: "", status: 1 });
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const actionPath = isEdit ? `${COMPLAINT_PATH}/update/${formData.complaint_code}/` : `${COMPLAINT_PATH}/create/`;
    ...
    if (result.success) {
      showModal(`Complaint ${isEdit ? "updated" : "created"} successfully!`);
      resetForm();
      refresh();
    } else {
  ...
  const handleDelete = async () => {
    ...
    const result = await deleteItem(`${COMPLAINT_PATH}/delete/${selectedRow.complaint_code}/`);
    if (result.success) {
  ...
  if (loading) return (
    <div className="loading-overlay dark:bg-slate-900/80">
      <div className="loading-spinner-container text-center">
        ...
        <p className="text-emerald-700 dark:text-emerald-500 font-bold">Loading Complaints...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="app-container min-h-screen transition-colors duration-300 dark:bg-slate-950 p-4">
        ...
        <div className="modal-overlay backdrop-blur-sm">
          <div className="modal-container dark:bg-slate-900 dark:border dark:border-slate-800">
            <div className="modal-body text-center p-8">
              ...
              <button className="bg-emerald-600 hover:bg-emerald-700 text-white w-full py-2.5 rounded-lg font-semibold transition-all" onClick={() => setModal({ ...modal, visible: false })}>OK</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8 bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border-l-4 border-emerald-500 transition-colors">
        <h4 className="text-2xl font-black text-gray-800 dark:text-slate-100 tracking-tight">Complaint Master</h4>

        {!showForm && (
          <div className="flex gap-2">
            <button className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-lg text-sm font-bold shadow-md shadow-emerald-200 dark:shadow-none transition-all" onClick={() => setShowForm(true)}>
              <FaPlus size={14} /> Add New
            </button>

            {selectedRow && (
              <div className="flex gap-2 animate-in slide-in-from-right-2">
                <button className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-5 py-2.5 rounded-lg text-sm font-bold shadow-md shadow-amber-200 dark:shadow-none transition-all"
                  onClick={() => { setFormData(selectedRow); setIsEdit(true); setShowForm(true); }}>
                  <FaEdit size={14} /> Edit
                </button>
                <button className="flex items-center gap-2 bg-rose-500 hover:bg-rose-600 text-white px-5 py-2.5 rounded-lg text-sm font-bold shadow-md shadow-rose-200 dark:shadow-none transition-all"
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm p-8 mb-8 border border-gray-100 dark:border-slate-800 animate-in zoom-in-95 duration-200">
          <h6 className="text-lg font-bold text-gray-800 dark:text-slate-100 mb-6 border-b dark:border-slate-800 pb-4">{isEdit ? "Update Complaint" : "Create Complaint"}</h6>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleSubmit}>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider ml-1">Complaint Code</label>
              <input
                ...
                value={formData.complaint_code} disabled={isEdit} required
                onChange={e => setFormData({ ...formData, complaint_code: e.target.value.toUpperCase() })}
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider ml-1">Complaint Name</label>
              <input
                ...
                value={formData.complaint_name} required
            ...
            <div className="md:col-span-2 flex justify-end gap-3 border-t dark:border-slate-800 border-gray-50 pt-6">
              <button className="bg-emerald-600 hover:bg-emerald-700 text-white px-12 py-2.5 rounded-lg font-bold shadow-lg shadow-emerald-200 dark:shadow-none transition-all">{isEdit ? "Update" : "Save"}</button>
      ...
      {!showForm && (
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden animate-in fade-in duration-500">
          {/* BaseCRUD components me agar dark mode support hai to theek, nahi to unhe wrapper me handle karna hoga */}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50 dark:bg-slate-800/50 border-b border-gray-100 dark:border-slate-800">
                  ...
                  <th className="px-6 py-4 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Complaint Code</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Complaint Name</th>
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
                {(paginatedData || []).map(row => (
                    <tr
                      key={row.complaint_code}
                      onClick={() =>
                        setSelectedRow(
                          selectedRow?.complaint_code === row.complaint_code ? null : row
                        )
                      }
                      className={`group cursor-pointer transition-colors duration-150 ${
                        selectedRow?.complaint_code === row.complaint_code
                    ...
                    >
                      ...
                      <td className="px-6 py-4 font-black text-gray-800 dark:text-slate-200 text-sm">
                        {row.complaint_code}
                      </td>
