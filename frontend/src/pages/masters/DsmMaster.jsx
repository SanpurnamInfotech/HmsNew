...
const DsmMaster = () => {
  ...
  const handleDelete = async () => {
    ...
    const result = await deleteItem(`${PATH}/delete/${selectedRow.dsm_code}/`);
  ...
  return (
    <div className="app-container">
      ...
      <div className="section-header">
        <h4 className="page-title">DSM Master</h4>
        {!showForm && (
      ...
      {showForm && (
        <div className="form-container animate-in zoom-in-95 duration-200">
          <h6 className="form-section-title uppercase tracking-tighter">
            {isEdit ? "Update DSM Record" : "Create New DSM"}
          </h6>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6" onSubmit={handleSubmit}>
            
            <div className="space-y-1.5">
              <label className="form-label">DSM Code</label>
              <input
                ...
                onChange={e => setFormData({ ...formData, dsm_code: e.target.value.toUpperCase() })}
              />
            </div>

            <div className="space-y-1.5">
              <label className="form-label">DSM Name</label>
              <input
                className="form-input w-full"
                value={formData.dsm_name} 
                required
                placeholder="Enter DSM Name"
                onChange={e => setFormData({ ...formData, dsm_name: e.target.value })}
              />
            </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr>
                  ...
                  <th className="text-admin-th">DSM Code</th>
                  <th className="text-admin-th">DSM Name</th>
              ...
              <tbody className="divide-y" style={{ borderColor: "var(--border-color)" }}>
                {paginatedData.length > 0 ? (
                  [...paginatedData]
                    ...
                    <tr 
                      key={row.dsm_code} 
                      onClick={() => setSelectedRow(selectedRow?.dsm_code === row.dsm_code ? null : row)}
                      className={`group cursor-pointer transition-colors ${selectedRow?.dsm_code === row.dsm_code ? "bg-emerald-500/10" : "hover:bg-emerald-500/5"}`}
                    >
                      <td className="px-6 py-4">
                        <div className={`selection-indicator ${selectedRow?.dsm_code === row.dsm_code ? "selection-indicator-active" : "group-hover:border-emerald-500/50"}`}>
                          {selectedRow?.dsm_code === row.dsm_code && <div className="selection-dot" />}
                        </div>
                      </td>
                      <td className="text-admin-td ">{row.dsm_code}</td>
                      <td className="text-admin-td">{row.dsm_name}</td>
                  ...
                  <tr>
                    <td colSpan="5" className="px-6 py-24 text-center">
                      <FaTruckLoading size={64} className="mb-6 mx-auto opacity-10 text-emerald-500 animate-pulse" />
