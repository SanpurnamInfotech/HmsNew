...
const ExpensesMst = () => {
  ...
  const { data, loading, refresh, createItem, updateItem, deleteItem } = useCrud(`${EXPENSES_PATH}/`);
  ...
  const resetForm = () => {
    ...
    setIsEdit(false);
    setSelectedRow(null);
    setFormData({ expenses_code: "", expenses_name: "", status: 1 });
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const actionPath = isEdit ? `${EXPENSES_PATH}/update/${formData.expenses_code}/` : `${EXPENSES_PATH}/create/`;
    ...
    if (result.success) {
      showModal(`Expense ${isEdit ? "updated" : "created"} successfully!`);
      resetForm();
      refresh();
    } else {
  ...
  const handleDelete = async () => {
    ...
    const result = await deleteItem(`${EXPENSES_PATH}/delete/${selectedRow.expenses_code}/`);
    if (result.success) {
  ...
  if (loading) return (
    <div className="loading-overlay">
      <div className="loading-spinner-container text-center">
        ...
        <p className="text-emerald-700 font-bold">Loading Expenses...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="app-container">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8 bg-white p-6 rounded-xl shadow-sm border-l-4 border-emerald-500">
        <h4 className="text-2xl font-black text-gray-800 tracking-tight">Expenses Master</h4>
        ...
        {!showForm && (
          <div className="flex gap-2">
            ...
            {selectedRow && (
              <>
                <button className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-5 py-2.5 rounded-lg text-sm font-bold"
                  onClick={() => { setFormData(selectedRow); setIsEdit(true); setShowForm(true); }}>
                  <FaEdit size={14} /> Edit
                </button>
                <button className="flex items-center gap-2 bg-rose-500 hover:bg-rose-600 text-white px-5 py-2.5 rounded-lg text-sm font-bold"
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm p-8 mb-8 border border-gray-100 animate-in zoom-in-95 duration-200">
          <h6 className="text-lg font-bold text-gray-800 mb-6 border-b pb-4">{isEdit ? "Update Expense" : "Create Expense"}</h6>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleSubmit}>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Expense Code</label>
              <input
                ...
                value={formData.expenses_code} disabled={isEdit} required
                onChange={e => setFormData({ ...formData, expenses_code: e.target.value.toUpperCase() })}
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Expense Name</label>
              <input
                ...
                value={formData.expenses_name} required
            ...
            <div className="md:col-span-2 flex justify-end gap-3 border-t border-gray-50 pt-6">
              <button className="bg-emerald-600 hover:bg-emerald-700 text-white px-12 py-2.5 rounded-lg font-bold">{isEdit ? "Update" : "Save"}</button>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  ...
                  <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest">Expense Code</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest">Expense Name</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {(paginatedData || []).map(row => (
                    <tr
                      key={row.expenses_code}
                      onClick={() =>
                        setSelectedRow(
                          selectedRow?.expenses_code === row.expenses_code ? null : row
                        )
                      }
                      className={`group cursor-pointer transition-colors duration-150 ${
                        selectedRow?.expenses_code === row.expenses_code
                          ? "bg-emerald-50/40"
                          : "hover:bg-gray-50/50"
                      }`}
                    >
