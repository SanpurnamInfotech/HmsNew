...
const MoodHistoryMst = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const actionPath = isEdit 
      ? `${MOOD_PATH}/update/${formData.mood_history_code}/` 
      : `${MOOD_PATH}/create/`;
    ...
    if (result.success) {
      showModal(`Mood ${isEdit ? "updated" : "created"} successfully!`);
      resetForm();
      refresh();
    } else {
  ...
  const handleDelete = async () => {
    ...
    const result = await deleteItem(`${MOOD_PATH}/delete/${selectedRow.mood_history_code}/`);
    if (result.success) {
      showModal("Mood deleted successfully!");
      setSelectedRow(null);
      refresh();
    } else {
  ...
  if (loading) return (
    <div className="loading-overlay">
      <div className="loading-spinner-container text-center">
        ...
        <p className="text-emerald-700 font-bold">Loading Mood History...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="app-container">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8 bg-white p-6 rounded-xl shadow-sm border-l-4 border-emerald-500">
        <h4 className="text-2xl font-black text-gray-800 tracking-tight">Mood History Master</h4>
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm p-8 mb-8 border border-gray-100 animate-in zoom-in-95 duration-200">
          <h6 className="text-lg font-bold text-gray-800 mb-6 border-b pb-4">{isEdit ? "Update Mood" : "Create Mood"}</h6>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleSubmit}>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Mood Code</label>
              <input
                ...
                value={formData.mood_history_code} disabled={isEdit} required
                onChange={e => setFormData({ ...formData, mood_history_code: e.target.value.toUpperCase() })}
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Mood Name</label>
              <input
                ...
                value={formData.mood_history_name} required
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  ...
                  <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest">Mood Code</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest">Mood Name</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {(paginatedData || []).map(row => (
                    <tr
                      key={row.mood_history_code}
                      onClick={() =>
                        setSelectedRow(
                          selectedRow?.mood_history_code === row.mood_history_code ? null : row
                        )
                      }
                      className={`group cursor-pointer transition-colors duration-150 ${
                        selectedRow?.mood_history_code === row.mood_history_code
                          ? "bg-emerald-50/40"
                          : "hover:bg-gray-50/50"
                      }`}
                    >
                      ...
                      <td className="px-6 py-4 font-black text-gray-800 text-sm">
                        {row.mood_history_code}
                      </td>

                      <td className="px-6 py-4 font-bold text-gray-700">
                        {row.mood_history_name}
                      </td>
