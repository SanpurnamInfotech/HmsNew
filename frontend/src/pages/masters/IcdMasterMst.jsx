...
const result = isEdit
  ...
  const handleDelete = async () => {
    ...
    if (result.success) {
      showModal("ICD deleted successfully!");
      setSelectedRow(null);
      refresh();
    } else {
  ...
  if (loading) return (
    <div className="loading-overlay">
      <div className="loading-spinner-container text-center">
        ...
        <p className="text-emerald-700 font-bold">Loading ICD Master...</p>
      </div>
    </div>
  );
    ...
    <div className="app-container">
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm p-8 mb-8 border border-gray-100 animate-in zoom-in-95 duration-200">
          <h6 className="text-lg font-bold text-gray-800 mb-6 border-b pb-4">{isEdit ? "Update ICD" : "Create ICD"}</h6>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleSubmit}>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">ICD Code</label>
              <input
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">ICD Name</label>
              <input
