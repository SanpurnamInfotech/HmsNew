...
const PrescriptionReport = () => {
    ...
    const handleSearch = async () => {
        ...
        setLoading(true);
        try {
            // Corrected: Using DataService.getAll with params for the report
    ...
    return (
        <div className="app-container">
            ...
            <div className="section-header">
                ...
                {reportData && (
                    <div className="flex gap-2">
                        <button className="btn-secondary flex items-center gap-2 bg-slate-700 text-white px-4 py-2 rounded-lg hover:bg-slate-600 transition-colors" 
            ...
            {/* FILTERS SECTION */}
            <div className="form-container mb-6 animate-in fade-in duration-500">
            ...
            {/* REPORT VIEW (The Area that gets turned into PDF) */}
            {reportData ? (
                <div ref={reportRef} id="printable-area" className="bg-white text-slate-900 rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-300 mx-auto max-w-4xl border border-slate-200">
                    <div className="p-8 text-center bg-slate-50 border-b border-slate-200">
                        <h2 className="text-3xl font-black text-slate-800 uppercase tracking-tighter mb-1">Saishwari Clinic</h2>
                        <p className="font-bold text-emerald-600 mb-1">Hospital For Mental Health</p>
                        <p className="text-sm text-slate-500">Miraj, Maharashtra | Contact: 8975582494</p>
                    </div>
                                        ...
                                        reportData.items.map((item, index) => (
                                            <tr key={index} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-4 py-4 font-bold text-slate-800">
                                                    {item.medicine_name || (item.medicine_code?.medicine_name) || item.medicine_code || 'N/A'}
                                                </td>
                        ...
                        <div className="mt-12 pt-8 border-t border-slate-100 flex justify-end">
                            <div className="text-center">
                                ...
                                <p className="text-lg font-black text-slate-800 uppercase tracking-tighter">
                                    {reportData.doctor_details?.name || (reportData.doctor_details && reportData.doctor_details.name) || 'Authorized Doctor'}
                                </p>
                                <p className="text-xs font-bold text-emerald-600">
                                    {reportData.doctor_details?.degree || (reportData.doctor_details && reportData.doctor_details.degree) || ''}
                                </p>
                            </div>
                        </div>
                    </div>
